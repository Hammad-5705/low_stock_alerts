...
#     # Read from globals to allow test overrides  
#     target_warehouses = globals().get('target_warehouses', ["test - B"])  # targeted wearhouses
#     recipient_email = globals().get('recipient_email', "hamadnad2014@gmail.com")  
    
#     if not target_warehouses or not recipient_email:    
#         frappe.logger().warning("Low Stock Alerts: target_warehouses or recipient_email not configured")    
#         return    
    
#     # Get reorder rules for target warehouses only  
#     placeholders = ", ".join(["%s"] * len(target_warehouses))  
#     reorder_data = frappe.db.sql(f"""    
#         SELECT    
...
#     """, (nowdate(), *target_warehouses), as_dict=True)    
...
#         projected_qty = flt(frappe.db.get_value("Bin", {"item_code": d.item_code, "warehouse": d.warehouse}, "projected_qty") or 0)    
...
#     if low_stock_items:    
#         send_low_stock_email(low_stock_items, recipient_email)    
    
# def send_low_stock_email(items, recipient):    
#     template = """    
#     <h2>Low Stock Alert</h2>    
#     <p>The following items are at or below reorder level in your monitored warehouses:</p>    
...
#     </table>    
#     """    
#     message = frappe.render_template(template, {"items": items})    
#     frappe.sendmail(    
